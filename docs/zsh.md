...
- Completion is case-insensitive, fuzzy (up to two typos), menu-driven and
  grouped by type.
- mise activates per-project runtimes from `.nvmrc`, `.node-version`,
  `.python-version`, `.tool-versions` or `.mise.toml`; brew node, go and java
  stay the defaults elsewhere. direnv loads a trusted `.envrc` on `cd`
  (`direnv allow` once per repo).
...
| `fproj` | Open a project directory as a tmux session (fzf over `DOTFILES_PROJECT_DIRS`, default `~/Projects`; creates or switches). tmux `prefix + o` runs it in a popup |
...
`DOTFILES_PROJECT_DIRS` (colon-separated) sets the roots `fproj` and
`prefix + o` list; the default is `~/Projects`.
...
| `zsh/30-tools.zsh` | Syntax-highlighting colors, Ghostty shell integration, zoxide, mise, direnv, delta pager, fzf options and key bindings |
